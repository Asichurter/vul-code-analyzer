...
@Trainer.register("ind_forward_optim_gradient_descent", constructor="from_partial_objects")
class IndependentForwardOptimGradientDescentTrainer(Trainer):
    """
    This trainer is adapted from "GradientDescentTrainer", with extra independent forward and
    optimization of each objective/task supported.
    ...
    def __init__(
        ...
        forward_step_list: List[str],
        forward_step_batch_sizes: Dict[str, int],
        ...
        **kwargs,
        ...
        self._forward_step_list = forward_step_list
        self._forward_step_batch_sizes = forward_step_batch_sizes
    ...
    def _split_batches_into_group(self, tensor_or_tensor_dict: Union[Dict, torch.Tensor], batch_size: int):
        if isinstance(tensor_or_tensor_dict, torch.Tensor):
            return torch.split(tensor_or_tensor_dict, batch_size)
        else:
            batches = []
            for key, val in tensor_or_tensor_dict.items():
                returns = self._split_batches_into_group(val, batch_size)
                if len(batches) == 0:
                    batches = [{key: ret} for ret in returns]
                else:
    ...
    def _reassign_batch_group_with_batch_size(self, batch_group: List[Dict[str, torch.Tensor]], batch_size: int) -> List[Dict[str, Optional[torch.Tensor]]]:
        """
            This method will first aggregate all the samples within this batch group (batch_size * accumulate_step),
            and reassign batch group by resplitting batches based on given new batch size.
            For instances, 8 * 16 -> 16 * 8.
        """
        # [ADAPTED NOTE]: Here we assume the "accumulated_step_num=1", and there is only one group in the batch groups.
        #                 So you may set the batch_size as large as possible.
        aggregate_batches = batch_group[0]
        reassigned_batch_group = self._split_batches_into_group(aggregate_batches, batch_size)
        return reassigned_batch_group
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        logger.warning(f'If you load all the batch instances by setting large batch_size and use "_reassign_batch_group_with_batch_size" '
                       f'to control the forward batch size, you must use the optimized token&line edges, as the ground-truth label graph '
                       f'will be constructed in-place rather than using padded ones which may cause size mismatch error. '
                       f'The root of this behavior is that code embedding is done within each forward step type independently.')
        ...
        for batch_group in batch_group_generator_tqdm:
            ...
            batch_loss = 0.0
            batch_group_outputs = [{} for _ in range(len(batch_group))]
            # ADAPTED NOTE: Here we wrap the forward/backward process with extra forwrad_step loop.
            # TODO: Validate this modification.
            for forward_step in self._forward_step_list:
                new_batch_group = self._reassign_batch_group_with_batch_size(batch_group, self._forward_step_batch_sizes[forward_step])

                # Grad must be cleared before forwarding every type
                self.optimizer.zero_grad()
                for batch_i, batch in enumerate(new_batch_group):
                    batch['forward_step_name'] = forward_step
                    ...
                    with amp.autocast(self._use_amp):
                        ...
                        loss = batch_outputs.pop("loss")
                        # batch_group_outputs[batch_i].update(batch_outputs)        # try to not update batch_outputs here, to check if tensors in them cause memory retaining
                        reg_loss = batch_outputs.get("reg_loss")
                        if torch.isnan(loss):
                            raise ValueError("nan loss encountered")
                        loss = loss / len(new_batch_group)

                        batch_loss += loss.item()
                        if reg_loss is not None:
                            reg_loss = reg_loss / len(new_batch_group)
                    ...
                    # Empty cache to free the memory of results from last forward type
                    # if forward_step in self._optim_mem_steps:
                    #     del batch_outputs
    ...
    def _validation_loss(self, epoch: int) -> Tuple[float, Optional[float], int]:
        ...
        try:
            ...
            done_early = False
            # TODO: Batch sizes will not be reassigned during validating...
            for batch in val_generator_tqdm:
                # [ADAPTED NOTE]: batch_loss should be the sum of the losses from all the types of forward.
                val_batch_loss = 0.0
                for forward_step in self._forward_step_list:
                    batch['forward_step_name'] = forward_step
                    if self._distributed:
                ...
                # [ADAPTED NOTE]: For original implementation, this var will increase when val_loss is accumulated once.
                #                 But here we assume there is always a "loss" item returned by model for convenience during
                #                 validation.
