...
from downstream.model.devign.data_loader.batch_graph import GGNNBatchGraph
from downstream.model.devign.devign_utils import load_default_identifiers, initialize_batch, debug, load_pickle

from downstream.model.devign.devign_global_flag import global_cuda_device


class DataEntry:
    def __init__(self, datset, num_nodes, features, edges, target, code=None):
        self.dataset = datset
...
class DataSet:
    ...
    def read_dataset(self, test_src, train_src, valid_src):
        ...
        train_data = load_pickle(train_src)
        for entry in tqdm(train_data):
            example = DataEntry(datset=self, num_nodes=len(entry[self.n_ident]), features=entry[self.n_ident],
                                edges=entry[self.g_ident], target=entry[self.l_ident], code=entry.get('code')) # target=entry[self.l_ident][0][0])
        ...
        if valid_src is not None:
            ...
            valid_data = load_pickle(valid_src)
        ...
        if test_src is not None:
            ...
            test_data = load_pickle(test_src)
