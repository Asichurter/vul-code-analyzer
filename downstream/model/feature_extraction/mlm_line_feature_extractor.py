...
class GradPredictor(Predictor):
    def __init__(self, model, reader, frozen=False):
            ...
            for name, output in list(outputs.items()):
                if isinstance(output, torch.Tensor):
                    ...
                    if output.size(0) != batch_size:
                        self._model._maybe_warn_for_unseparable_batches(name)
                        continue
...
class MLMLineExtractor(GradPredictor):
    def __init__(self, model_archive_path,
        ...
        super().__init__(model, reader, frozen=frozen)
