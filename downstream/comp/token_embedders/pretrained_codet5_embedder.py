...
@TokenEmbedder.register("pretrained_codet5")
class PretrainedCodeT5Embedder(TokenEmbedder):
    """
    Adapted version from `PretrainedTransformerEmbedder`, with decoder_input_ids = encoder_input_ids, for 
    embedding of understanding tasks.
    ...
    def __init__(
        ...
        *,
        ...
        self.squeeze_seq_dim = squeeze_seq_dim
    ...
    def forward(
        ...
        parameters = {"input_ids": token_ids, "attention_mask": transformer_mask.float(),
                      "decoder_attention_mask": transformer_mask.float(),                   # We add three more parameters as https://github.com/salesforce/CodeT5/blob/afcc8efd4accc0a44998f9c400332579be819773/models.py#L134
        ...
        if self._scalar_mix is not None:
            ...
            raise ValueError(f'Not support scalar mix of outputs from all layers')
        else:
            embeddings = self._extract_vec_features_from_outputs(transformer_output, token_ids)
    ...
    # The function we add to adapt outputs from CodeT5.
    def _extract_vec_features_from_outputs(self, outputs, token_ids):
        decoder_hidden_states = outputs['decoder_hidden_states'][-1]
        eos_mask = token_ids.eq(self.eos_token_id)

        if len(torch.unique(eos_mask.sum(1))) > 1:
            raise ValueError("All examples must have the same number of <eos> tokens.")

        # Here we adopt the eos embedding from last layer of decoder outputs as embedding,
        # following CodeT5 on defect task:
        # https://github.com/salesforce/CodeT5/blob/afcc8efd4accc0a44998f9c400332579be819773/models.py#L141
        embeddings = decoder_hidden_states[eos_mask, :].view(decoder_hidden_states.size(0), -1,
                                                             decoder_hidden_states.size(-1))[:, -1, :]
        if self.squeeze_seq_dim:
