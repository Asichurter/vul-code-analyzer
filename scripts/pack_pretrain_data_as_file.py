...
for vol in range(0,20):
    ...
    vol_base_path = f'/data1/zhijietang/vul_data/datasets/docker/packed_data_jsons_new/vol{vol}/'
    print(f'Reading Vol.{vol}...')
    for item in tqdm(os.listdir(vol_base_path)):
        item_path = os.path.join(vol_base_path, item)
        data = load_json(item_path)
        data['id'] = item.split('.')[0]
