...
def extract_line_num(v_label):
    ...
    if matched is None:
        logger.warning(
            f"v_label={v_label} does not match the line number pattern")  # handle the cases like vol0/217.cpp
        return None
...
if __name__ == '__main__':
    ...
    path_prefix = '/data1/zhijietang/vul_data/datasets/docker/'  # '/root/data/'
    dot_json_path = path_prefix + 'test_temp/test5_dot.json'
    code_file_path = path_prefix + 'test_temp/test5.c'
    dump_json_path = path_prefix + 'test_temp/test5_packed.json'
    main_temp_bin_path = path_prefix + 'test_temp/main_temp.bin'
    main_temp_dot_dir = path_prefix + 'test_temp/dot'
    # os.system(f'rm -rf {main_temp_dot_dir}')
